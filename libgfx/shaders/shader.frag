	...
	for(int i = 0; i < MAXLIGHTS; i++)
	{
		if(light[i].lightPos != vec4(0.0))
		{
			...
			{			
				//DIRECTIONAL
				/*Ambient*/
				vec3 ambient = light[i].ambient * ambientStrength;

				/*Diffuse*/
				vec3 norm = normalize(Normal);
				vec3 lightDir = normalize(-light[i].lightPos.xyz);
				float diff = max(dot(norm, lightDir), 0.0);
				vec3 diffuse = diff * diffuseStrength * light[i].diffuse;

				/*Specular*/
				vec3 viewDir = normalize(viewPos - fragPos);
				vec3 reflectDir = reflect(-lightDir, norm);  
				float spec = pow(max(dot(viewDir, reflectDir), 0.0), shininess);
				vec3 specular = specularStrength * spec * light[i].specular;
			...
			{ 
				if(light[i].bIsPoint == true)
				{
					...
					result += ((ambient * intensity) + (diffuse * intensity) + (specular * intensity)) * attenuation;
				}
			}
		}
		outColor = texture(texSampler, fragTexCoord) * vec4(result, 1.0f);
